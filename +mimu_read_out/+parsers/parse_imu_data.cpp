...
//	const int nof_imus = *reinterpret_cast<unsigned char*>( mxGetData(prhs[1]) );
	const int PAYLOAD_SIZE = 4+4*6;
	const int PACKET_SIZE = 4+PAYLOAD_SIZE+2;
...
//	mexPrintf( filename );
	...
	const int nof_data_values = 6;
	const int data_values_size = 4 * nof_data_values;
...
//				mexPrintf("Read complete buffer %u\n",bytes_read);
	...
	plhs[2] = mxCreateNumericMatrix( 1, elem_count*PAYLOAD_SIZE, mxUINT8_CLASS ,mxREAL);
	...
	for( int i=0; i < size_inertial_data; i+=data_values_size ){
		for (int j=0; j<data_values_size; j+=4) {
			output_mat[i+j+0] = inertial_data[i+j+3];
			output_mat[i+j+1] = inertial_data[i+j+2];
			output_mat[i+j+2] = inertial_data[i+j+1];
			output_mat[i+j+3] = inertial_data[i+j];
		}
	}
...
const char* check_input_output(int nlhs, mxArray *plhs[ ], int nrhs, const mxArray *prhs[ ]){
	...
	if( nrhs != 1 )
		return "one input (filename) required";
	if( !mxIsChar( prhs[0] ) )
